...
def create_main_and_layer_diagrams(metadata_file_path, output_folder):
    ...
    for layer_name, layer_info in metadata.items():
        ...
        if cat not in categories:
            categories[cat] = []
        categories[cat].append((layer_name, layer_info.get('feature_types', {}), layer_info.get('features', {})))

    # Generate one diagram per category
    for cat, layers in categories.items():
        dot = Digraph(comment=f'{cat} Hierarchy', format='png')
        dot.attr(rankdir='LR', size='8')
        dot.node(cat, cat, shape='box', style='filled', color=category_colors.get(cat, 'white'))

        dot_detailed = Digraph(comment=f'{cat} Hierarchy', format='png')
        dot_detailed.attr(rankdir='LR', size='8')
        dot_detailed.node(cat, cat, shape='box', style='filled', color=category_colors.get(cat, 'white'))
        ...
        for layer_name, feature_types, features in layers:
            ...
            dot.node(layer_name, layer_name, shape='box', style='filled', fillcolor='lightblue')
            dot.edge(cat, layer_name)
            dot.edge(layer_name, f"{layer_name}_themes")
            dot.node(f"{layer_name}_themes", themes_str, shape='box', style='filled', fillcolor='lightgray')

            dot_detailed.node(layer_name, layer_name, shape='box', style='filled', fillcolor='lightblue')
            dot_detailed.edge(cat, layer_name)
            dot_detailed.edge(layer_name, f"{layer_name}_themes")
            dot_detailed.node(f"{layer_name}_themes", themes_str, shape='box', style='filled', fillcolor='lightgray')
            feature_types_str = ', '.join(feature_types) if isinstance(feature_types, list) else str(feature_types)
            dot_detailed.node(f"{layer_name}_feature_types", feature_types_str, shape='box', style='filled', fillcolor='lightyellow')
