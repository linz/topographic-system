    ...
    for _, row in themes_df.iterrows():
        ...
        if layer_desc_df is not None:
            ...
            if not matching_desc.empty:
                ...
                model_summary_picture_path = os.path.join(model_themes_folder, f"{category}_summary_diagram.svg")
                model_theme_picture_path = os.path.join(model_themes_folder, f"{category}_diagram.svg")

        if layer_category not in summary_dictionary:
            summary_dictionary[category] = [layer_category, model_summary_picture_path, model_theme_picture_path]
        ...
        table_dictionary[table]['features'][feature_type] = {
            ...
            'feature_picture_path': os.path.join(layer_pictures_folder, f"{table_lds}.gif"),
            'aerial_picture_path': os.path.join(aerial_pictures_folder, f"{table_lds}.gif")
        }
...
def create_html_file(table_dictionary, html_output_folder):
        ...
        for category_name, tables in table_dictionary.items():
            ...
            html_content += f'<p><strong>Total feature types:</strong> {len(feature_types)}</p>\n'
            html_content += f'<p><strong>Layer Category:</strong> {tables["layer_category"]}</p>\n'
        ...
        # Write HTML file
        html_file_path = os.path.join(html_output_folder, "topographic_layers_metadata.html")
        ...
        <body>
            ...
            <div class="stats">
                <h3>Overview</h3>
                <p><strong>Total Tables:</strong> """ + str(len(table_dictionary)) + """</p>
                <p><strong>Total Feature Types:</strong> """ + str(sum(len(table_info['feature_types']) for table_info in table_dictionary.values())) + """</p>
            </div>
...
if __name__ == "__main__":
    ...
    layer_descriptions_file = os.path.join(source_metadata_folder, "layer_descriptions.csv")
    ...
    table_dictionary, summary_dictionary = create_features_dict(conn, 
                                            layer_descriptions_file, 
                                            features_bythemes_file, 
                                            schema_fields_folder, 
                                            layer_pictures_folder, 
