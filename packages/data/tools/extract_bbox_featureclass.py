...
@staticmethod
def write_dataset(extension, gdf, output_file, layer_name=None, dataset_name=None, append_data=True):
    try:
        if extension == "geojson":
...
bbox = (
    1200000, 4975000, 1380000, 5100000
)
...
for layer in layers:
    ...
    if gdf is not None:
        write_dataset("OpenFileGDB", gdf, output_file, layer_name=layer, dataset_name=datasets_dict[layer], append_data=True)
