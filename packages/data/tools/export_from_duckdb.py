...
reordered_full = [f for f in field_order if f in in_order] + [
    f for f in in_order if f not in field_order
]
print(reordered_full)

# Define PostGIS connection string (update with your credentials)
postgis_conn_str = (
    "host=localhost dbname=topo50 user=postgres password=landinformation port=5432"
)

# Query buildings.building table
# query = (
#     "id, macronated, building_use, name_ascii, t50_fid, name, status, theme, feature_type, object_name, shape_area, "
#     "geometry, "
...
query = (
    "id, ST_Transform(ST_GeomFromWKB(geometry),'EPSG:4167', 'EPSG:4167') as geometry"
)
