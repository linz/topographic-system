...
for table in tables:
    if (
        table[0].startswith("gpkg")
        or table[0].startswith("rtree")
        or table[0].startswith("sqlite_sequence")
    ):
        exclude += table[0] + " "
        continue
...
output_string = (
    f"{connection} --exclude-tables {exclude} {output_diagrams}\\full_database.svg\n"
)
output_string2 = (
    f"{connection} --exclude-tables {exclude} {output_diagrams}\\full_database.pdf\n"
)
output_string3 = (
    f"{connection} --exclude-tables {exclude} {output_diagrams}\\full_database.er\n"
)
with open(output_bat_file, "a") as f:
    f.write(output_string)
    f.write(output_string2)
