...
class CartoTextProcessor:
    ...
    def read_and_process_tabs(self, mapping_spreadsheet, full_layers_sheet):
        ...
        """
        try:
            self.logger.info(f"Reading Excel file: {mapping_spreadsheet}, sheet: {full_layers_sheet}")
            ...
            for index, row in df.iterrows():
                row_value = str(row[layer_column]).strip() if pd.notna(row[layer_column]) else ""
            ...
            for section in att_sections:
                section_row = self.process_att_section(section['att_name'], section['data'])
                
                # Append section_row to formatted_rows
                if section_row is not None and not section_row.empty:
                    formatted_rows = pd.concat([formatted_rows, section_row], ignore_index=True)
                    self.logger.info(f"Added {len(section_row)} rows from {section['att_name']} to formatted_rows")
    ...
    def process_att_section(self, att_name, section_data):
        ...
        if len(section_data) >= 4:
            ...
            for row in [att_header_row, data_row1, data_row2, data_row3]:
                if 'SYMBOL' in section_df.columns:
                    ...
                    symbol_value = str(row.iloc[symbol_col_index]) if pd.notna(row.iloc[symbol_col_index]) else ""
            ...
            if symbol_values:
                ...
                self.logger.warning("No valid SYMBOL values found, keeping original ATT header row")
                section_data = [att_header_row]
        else:
            self.logger.warning(f"Section has only {len(section_data)} rows, expected at least 4 for processing")
        ...
        for col in section_df.columns:
            if col not in ['LAYER', 'SECTION']:
                section_df[col] = section_df[col].astype(str).str.replace("'", "", regex=False)
    ...
    def process_new_values_tab(self, mapping_spreadsheet, new_values_sheet):
        ...
        try:
            ...
            if 'Text Bend' in df.columns:
                ...
                empty_count = df['Text Bend'].isna().sum() + (df['Text Bend'] == '').sum()
                self.logger.info(f"Found {empty_count} empty values in 'Text Bend' column")
    ...
    def process_fulllayers_tab(self, mapping_spreadsheet, full_layers_sheet):
        ...
        """
        formatted_rows = self.read_and_process_tabs(mapping_spreadsheet, full_layers_sheet)
    ...
    def validate_and_assign_field(self, gdf, field_name, values):
        ...
        if field_type == 'string':
            ...
            if max_length and original_max > max_length:
                self.logger.warning(f"Truncated {field_name} values to {max_length} characters")
        ...
        gdf[field_name] = values
        self.logger.info(f"Assigned values to {field_name} with type {field_type}" + 
    ...
    def _enforce_field_constraints(self, gdf):
        ...
        for field_name, (field_type, max_length) in gdf._field_specs.items():
            if field_type == 'string' and max_length is not None:
                ...
                original_length = gdf[field_name].str.len().max() if not gdf[field_name].isna().all() else 0
                gdf[field_name] = enforce_string_length(gdf[field_name], max_length)
                if original_length > max_length:
                    self.logger.warning(f"Truncated {field_name} values from max {original_length} to {max_length} characters")
            elif field_type == 'float64':
                # Ensure float64 type
            ...
            for field, value in conditions.items():
                if field == 'text_place':
                    ...
                    if isinstance(value, str) and ',' in value:
                        values = value.split(',')
                        query_parts.append('(' + ' or '.join([f"{field} == {v}" for v in values]) + ')')
                    else:
                ...
                try:
                    ...
                    for field, value in conditions.items():
                        if field in gdf.columns:
                            ...
                            unique_vals = gdf[field].unique()[:10]  # Show first 10 unique values
                            self.logger.info(f"    Field '{field}' exists, type: {field_type}, sample values: {unique_vals}")
                        else:
                            self.logger.warning(f"    Field '{field}' does not exist in GDF")
        
        return queries_processed

    def update_layer_fields_from_queries(self, gdf, processed_queries, full_layers, new_values, font_mapping, new_font_name):
        """
        ...
        for query_idx, query_data in enumerate(processed_queries):
            self.logger.info(f"Processing query {query_idx + 1}: {query_data['symbol_text']}")
            ...
            self.logger.info(f"  Matched {len(matched_features)} features for this index query")
            ...
            for text_bend_value, group in text_bend_groups:
                self.logger.info(f"    Processing text_bend = {text_bend_value} ({len(group)} features)")
                
                # Query new_values_processed using text_bend value
                matching_new_values = new_values[new_values['Text Bend'] == text_bend_value]
                if matching_new_values.empty:
                    self.logger.warning(f"      No matching new_values for text_bend = {text_bend_value}")
                ...
                matching_new_values = matching_new_values[matching_new_values['Text Height'] == text_height]
                if matching_new_values.empty:
                    self.logger.warning(f"      No matching new_values for Text Height = {text_height}")
                ...
                if style_value:
                    ...
                    gdf.loc[group_indices, 'size'] = pd.to_numeric(new_values_row.get('Size', 0), errors='coerce') 
                    gdf.loc[group_indices, 'offset'] = pd.to_numeric(new_values_row.get('Offset', 0), errors='coerce')
                    gdf.loc[group_indices, 'placement'] = new_values_row.get('Placement', '')
                    gdf.loc[group_indices, 'textanchor'] = new_values_row.get('Text Anchor', '')
                    gdf.loc[group_indices, 'labelanchor'] = pd.to_numeric(new_values_row.get('LabelAnchor', 0), errors='coerce')
                    gdf.loc[group_indices, 'charplace'] = new_values_row.get('Char Place', '')
                    gdf.loc[group_indices, 'chardistance'] = pd.to_numeric(new_values_row.get('Char Dist', 0), errors='coerce')
                    
                    self.logger.info(f"      Updated {len(group_indices)} features with:")
                    self.logger.info(f"        FONT: {new_font_name}")
                    self.logger.info(f"        STYLE: {style_value}")
                    self.logger.info(f"        COLOUR: {new_values_row.get('Colour', '')}")
                    self.logger.info(f"        SIZE: {new_values_row.get('Size', 0.0)}")
                    self.logger.info(f"        OFFSET: {new_values_row.get('Offset', 0.0)}")
                    self.logger.info(f"        PLACEMENT: {new_values_row.get('Placement', '')}")
                    self.logger.info(f"        TEXT ANCHOR: {new_values_row.get('Text Anchor', '')}")
                    self.logger.info(f"        LABEL ANCHOR: {new_values_row.get('LabelAnchor', 0.0)}")
                    self.logger.info(f"        CHAR PLACE: {new_values_row.get('Char Place', '')}")
                    self.logger.info(f"        CHAR DISTANCE: {new_values_row.get('Char Dist', 0.0)}")
                else:
                    self.unmatched_ids.append('Setting Zeros: ' + field_values['layer_id'] + ' - ' + value_setting_info)
                    ...
                    self.logger.info(f"      Set {len(group_indices)} features to null values")
        
        self.logger.info("Completed complex field updates")
        return gdf

    def process_carto_text_layer(self, carto_text_folder, product_database, carto_text_layer, 
        ...
        try:
            ...
            self.logger.info("Adding new fields to carto_text layer with type constraints...")
            ...
            for field_name, field_type, max_length in self.field_specs:
                if field_type == 'string':
                    ...
                    self.logger.info(f"Added {field_name}: string, max length {max_length}")
                elif field_type == 'float64':
            ...
            gdf._field_specs = {name: (dtype, max_len) for name, dtype, max_len in self.field_specs}
            ...
            if full_layers is not None and 'SYMBOL' in full_layers.columns:
                ...
                processed_queries = self.process_symbol_text_to_queries(full_layers, gdf)
                self.logger.info(f"Generated {len(processed_queries)} queries from SYMBOL text")
                ...
                gdf = self.update_layer_fields_from_queries(gdf, processed_queries, full_layers, new_values, font_mapping, new_font_name)
            else:
                self.logger.warning("Cannot process SYMBOL text - full_layers CSV not available or SYMBOL column missing")
                processed_queries = []
            
            
            # Example: Display basic statistics
            self.logger.info(f"Carto text layer geometry types: {gdf.geometry.type.value_counts().to_dict()}")
            ...
            if hasattr(gdf, '_symbol_queries') and gdf._symbol_queries:
                ...
                successful_queries = len([q for q in gdf._symbol_queries if q['matched_features'] > 0])
                ...
                for i, query in enumerate(gdf._symbol_queries[:3]):  # Show first 3
                    ...
                    self.logger.info(f"     -> {query['query_string']} (matched {query['matched_features']} features)")
            ...
            for field_name, (field_type, max_length) in gdf._field_specs.items():
                if field_type == 'string':
                    schema_properties[field_name] = f'str:{max_length}' if max_length else 'str'
                elif field_type == 'float64':
    ...
    def run(self, mapping_spreadsheet, full_layers_sheet, new_values_sheet, font_mapping_sheet,
            carto_text_folder, product_database, carto_text_layer, new_font_name, export_format='GPKG'):
        """
        ...
        full_layers_csv = self.process_fulllayers_tab(mapping_spreadsheet, full_layers_sheet)
        if full_layers_csv:
        ...
        new_values_csv = self.process_new_values_tab(mapping_spreadsheet, new_values_sheet)
        if new_values_csv:
        ...
        font_mapping_csv = self.process_font_mapping_tab(mapping_spreadsheet, font_mapping_sheet)
        if font_mapping_csv:
