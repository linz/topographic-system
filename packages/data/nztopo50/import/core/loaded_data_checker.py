...
topographic_data_layers: dict[str, list[str]] = {
    ...
    "structure": ["dry_dock", "fish_farm", "marine_farm", "reservoir", "siphon", "tank"],
    "vegetation": ["exotic", "native", "orchard", "scattered_scrub", "scrub", "vineyard"],
    "fence_line": ["fence"],
    ...
    "physical_infrastructure_line": ["pipeline", "powerline", "telephone", "walkwire", "water_race"],
...
def test_coordinate_system_conversions(db_tables: DBTables) -> dict[str, Any]:
    ...
    """
    conn = db_tables.get_connection()
    results: dict[str, Any] = {"test_name": "Coordinate System Conversions", "details": []}
...
def test_field_management(db_tables: DBTables) -> dict[str, Any]:
    ...
    required_fields = ["feature_type", "topo_id", "capture_method", "change_type", "update_date", "create_date", "version"]
    ...
    try:
        ...
        all_passed = all(d["passed"] for d in results["details"])  # type: ignore
        results["status"] = "PASS" if all_passed else "FAIL"
        results["message"] = f"Verified {len(results['details'])} tables for required fields"
    except Exception as e:
        results["status"] = "ERROR"
        results["message"] = str(e)
...
def test_data_cleanup(db_tables: DBTables) -> dict[str, Any]:
    ...
    try:
        for schema in ["release64", "carto"]:
            ...
            for table in tables:
                ...
                with conn.cursor() as cur:
                    ...
                    geom_is_last = last_col and last_col[0] in ["geom", "geometry", "wkb_geometry"]
        ...
        all_passed = all(d["passed"] for d in results["details"])  # type: ignore
        results["status"] = "PASS" if all_passed else "FAIL"
        results["message"] = f"Checked {len(results['details'])} tables for cleanup issues"
    except Exception as e:
        results["status"] = "ERROR"
        results["message"] = str(e)
...
def test_data_quality_corrections(db_tables: DBTables) -> dict[str, Any]:
    ...
    try:
        ...
        if db_tables.table_exists("release64", "tunnel_line") and db_tables.column_exists("release64", "tunnel_line", "tunnel_use"):
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM release64.tunnel_line WHERE tunnel_use = 'ivestock'")
        ...
        if db_tables.table_exists("release64", "trig_point") and db_tables.column_exists("release64", "trig_point", "trig_type"):
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(DISTINCT trig_type) FROM release64.trig_point WHERE trig_type IS NOT NULL")
        ...
        if db_tables.table_exists("release64", "road_line") and db_tables.column_exists("release64", "road_line", "way_count"):
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM release64.road_line WHERE way_count = '1' AND way_count != 'one way'")
...
def test_name_field_additions(db_tables: DBTables) -> dict[str, Any]:
    ...
    try:
        ...
        all_passed = all(d["passed"] for d in results["details"])  # type: ignore
        results["status"] = "PASS" if all_passed else "FAIL"
        results["message"] = f"Verified {len(results['details'])} layers for name fields"
    except Exception as e:
        results["status"] = "ERROR"
        results["message"] = str(e)
...
def test_default_value_assignment(db_tables: DBTables) -> dict[str, Any]:
    ...
    try:
        ...
        if db_tables.table_exists("release64", "runway"):
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM release64.runway WHERE surface IS NULL")
        ...
        if db_tables.table_exists("release64", "railway_line"):
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM release64.railway_line WHERE vehicle_type IS NULL")
                null_count = cur.fetchone()[0]
            checks.append({
...
def test_metadata_schema(db_tables: DBTables) -> dict[str, Any]:
    ...
    try:
        ...
        results["status"] = "PASS" if has_defaults and len(results["details"]) > 0 else "FAIL"
        results["message"] = f"Verified metadata schema on {len(results['details'])} field definitions"
    except Exception as e:
        results["status"] = "ERROR"
        results["message"] = str(e)
...
def create_layer_failures_report(
...
if __name__ == "__main__":
    ...
    workflow_report.to_csv(os.path.join(log_folder, "workflow_tests_report.csv"), index=False)
