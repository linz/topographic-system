...
class GeoParquetTableDiffComparer:
    ...
    def __init__(
        ...
        self.current_release_name = current_release_name
        self.previous_release_name = previous_release_name
        self.release_date = release_date
        self.current_release_path = current_release_path
    ...
    def _is_http_path(self, path: str) -> bool:
        lower_path = path.lower()
        return lower_path.startswith("http://") or lower_path.startswith("https://")

    def _is_remote_path(self, path: str) -> bool:
        return self._is_s3_path(path) or self._is_http_path(path)

    def _is_parquet_file_path(self, path: str) -> bool:
        return path.lower().endswith(".parquet")

    def configure_s3_from_env(self):
        """Enable DuckDB remote file access with optional AWS-style environment variables."""
    ...
    def list_release_tables(self, release_path: str) -> dict[str, str]:
        ...
        if self._is_parquet_file_path(release_path):
            table_name = os.path.splitext(os.path.basename(release_path))[0]
            return {table_name: release_path}

        if self._is_remote_path(release_path):
    ...
    def run(self) -> None:
        """Run end-to-end comparison and changelog export for configured releases."""
        try:
            if self._is_remote_path(self.current_release_path) or self._is_remote_path(self.previous_release_path):
                self.configure_s3_from_env()

            if not os.path.exists(self.change_logs_path):
                os.makedirs(self.change_logs_path)

            current_tables = self.list_release_tables(self.current_release_path)
            previous_tables = self.list_release_tables(self.previous_release_path)
            ...
            for idx, table in enumerate(common_tables, start=1):
                ...
                added_dfs.append(insert_df)
                removed_dfs.append(delete_df)
                updated_dfs.append(update_df)

            all_dfs = [
                pd.concat(added_dfs, ignore_index=True),
                pd.concat(removed_dfs, ignore_index=True),
                pd.concat(updated_dfs, ignore_index=True),
            ]
            ...
            for i, df in enumerate(all_dfs):
                ...
                if not self.use_hive_partitioning:
                    output_file = os.path.join(
                        self.change_logs_path,
                        self.current_release_name,
                        f"{self.current_release_name}_{change_type}_changelog.parquet",
                    )
                    os.makedirs(os.path.dirname(output_file), exist_ok=True)
                    df.to_parquet(output_file, index=False)
                    print(f"Exported {change_type} changes to {output_file}")
                else:
                    partition_path = os.path.join(self.change_logs_path, self.current_release_name)
...
def main_s3() -> None:
    ...
    comparator = GeoParquetTableDiffComparer(
        ...
        previous_release_name="release62",
        release_date="2025-09-25",
        current_release_path=r"https://d1jzh93b1t1cv.cloudfront.net/data/building/pull_request/pr-22/building.parquet",
        previous_release_path=r"https://d1jzh93b1t1cv.cloudfront.net/data/building/latest/building.parquet",
        use_hive_partitioning=False,
        change_logs_path=r"c:/temp/data-changes",
    )
