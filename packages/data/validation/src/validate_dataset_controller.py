...
class ValidateDatasetController:
    ...
    def default_validation_summary_dictionary(self):
        return {
            ...
            "query_rules": False,
            "is_multipart_about": "If True - a feature is found that is multipart.",
            "is_multipart": False
        }
    ...
    def run_process_queries(self):
        ...
        for query_rule in self.settings.query_rules:
            ...
            validator.run_query_rule_checks(rule_name='query_rule', rule=query_rule["rule"], column_name=query_rule["column"])
            self.summary_report = validator.summary_report
    ...
    def run_process_features_on_layer(self):
        for layer in self.settings.feature_in_layers:
            ...
            print(f"Running feature on layer check between {layer['table']} and {layer['intersection_table']}")
        ...
        for layer in self.settings.feature_not_on_layers:
            ...
            print(f"Running feature not on layer check between {layer['table']} and {layer['intersection_table']}")
        ...
        for layer in self.settings.line_not_on_feature_layers:
            ...
            print(f"Running line not on feature layer check between {layer['line_table']} and {layer['intersection_table']}")

            validator.set_exports(export_parquet=self.settings.export_parquet, 
                                  export_parquet_by_geometry_type=self.settings.export_parquet_by_geometry_type, 
                                  export_gpkg=self.settings.export_gpkg)
            validator.run_layer_intersections(rule_name='line_not_on_feature_layers', intersect=False, buffer_lines=True)
            self.summary_report = validator.summary_report
        ...
        for layer in self.settings.line_not_touches_feature_layers:
            ...
            print(f"Running line not touches feature layer check between {layer['line_table']} and {layer['intersection_table']}")
            
            validator.set_exports(export_parquet=self.settings.export_parquet, 
                                  export_parquet_by_geometry_type=self.settings.export_parquet_by_geometry_type, 
                                  export_gpkg=self.settings.export_gpkg)
            validator.run_layer_intersections(rule_name='line_not_touches_feature_layers', intersect=False, buffer_lines=True, predicate='touches')
            self.summary_report = validator.summary_report
        ...
        for layer in self.settings.feature_not_contains_layers:
            ...
            print(f"Running feature not contains layer check between {layer['table']} and {layer['intersection_table']}")
            
            validator.set_exports(export_parquet=self.settings.export_parquet, 
                                  export_parquet_by_geometry_type=self.settings.export_parquet_by_geometry_type, 
                                  export_gpkg=self.settings.export_gpkg)
            validator.run_layer_intersections(rule_name='feature_not_contains_layers', intersect=False, buffer_lines=False, predicate='contains')
            self.summary_report = validator.summary_report
    ...
    def run_process_self_intersections(self):
        for layer in self.settings.self_intersect_layers:
            ...
            validator = self.validator.create_validator(summary_report=self.summary_report, 
                                                        export_validation_data=self.settings.export_validation_data,
                                                        table=table, table2=None, export_layername=export_layername, where_condition=where_condition, message=message)
            validator.set_exports(export_parquet=self.settings.export_parquet, 
                                    export_parquet_by_geometry_type=self.settings.export_parquet_by_geometry_type, 
                                    export_gpkg=self.settings.export_gpkg)
