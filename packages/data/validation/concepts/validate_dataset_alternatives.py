...
class TopologyValidator:
    def __init__(self, db_url, table, export_layername, table2=None,  
                 ...
                 output_dir=r"c:\data\topoedit\validation-data", area_crs=2193):
        ...
        if db_url.startswith('postgresql'):
            ...
            spatialite_path = os.environ.get('USERPROFILE') + r"\AppData\Local\miniconda3\Library\bin"
            self.set_mod_spatialite(spatialite_path)
            self.engine = sqlite3.connect(db_url)
            os.chdir(self.spatialite_path)   # Temporarily switch working directory to Spatialite extensions path
    ...
    def read_data_intersect(self):   
        if self.where_condition:
            if self.source == 'gpkg':
                ...
                if self.mode == 'twotable':
                    self.gdf2 = gpd.read_postgis(f"SELECT * FROM {self.table2}", self.engine, geom_col=self.geom_column)
        elif self.bbox:
            if self.source == 'gpkg':
            ...
            if self.source == 'gpkg':
                ...
                query = f"""
                   SELECT  a.*
                    ...
                    AND a.topo_id != b.topo_id;                    
                    """
                self.gdf = gpd.read_postgis(f"SELECT * FROM {self.table}", self.engine, geom_col=self.geom_column)
                if self.mode == 'twotable':
                    self.gdf2 = gpd.read_postgis(f"SELECT * FROM {self.table2}", self.engine, geom_col=self.geom_column)
    ...
    def read_data_where_column_null(self, column_name):
        ...
        if self.source == 'gpkg':
            ...
            self.gdf = gpd.read_file(self.db_url, layer=self.table, columns=[self.pkey, self.geom_column], where=where)
        else:
    ...
    def find_not_intersections_touches_on_geom(self, buffer_lines=True):
        ...
        intersecting_features = gpd.sjoin(self.gdf, self.gdf2, how='left', predicate='touches')
        non_intersecting_features = intersecting_features[intersecting_features['index_right'].isna()]
        non_intersecting_features.columns = [col.replace('_left', '') for col in non_intersecting_features.columns]
        columns = [self.pkey, self.geom_column]
        if self.pkey != 'topo_id':
    ...
    def find_not_contains_geom(self):
        ...
        intersecting_features = gpd.sjoin(self.gdf, self.gdf2, how='left', predicate='contains')
        non_intersecting_features = intersecting_features[intersecting_features['index_right'].isna()]
        non_intersecting_features.columns = [col.replace('_left', '') for col in non_intersecting_features.columns]
        columns = [self.pkey, self.geom_column]
        if self.pkey != 'topo_id':
    ...
    def save_gdf(self, gdf):
        ...
        if self.export_parquet:
            gdf.to_parquet(fr"{self.output_dir}\{self.layername}.parquet", engine='pyarrow', compression='zstd', write_covering_bbox=True, row_group_size=50000)
        if self.export_gpkg:
            gdf.to_file(fr"{self.output_dir}\topology_checks.gpkg", layer=f"{self.layername}", driver="GPKG", append=True)
    ...
    def save_gdf_outputs(self, column_name, check_name ='null'):
        ...
        if self.export_parquet:
            self.gdf.to_parquet(fr"{self.output_dir}\{self.layername}_{check_name}_{column_name}.parquet", engine='pyarrow', compression='zstd', write_covering_bbox=True, row_group_size=50000)
        if self.export_gpkg:
            self.gdf.to_file(fr"{self.output_dir}\topology_{check_name}_checks.gpkg", layer=f"{self.layername}_{check_name}_{column_name}", driver="GPKG", append=True)
    ...
    def save_intersection_outputs(self, intersection_geometries, intersection_geometries_point, intersection_geometries_line, intersection_geometries_multipolygon):
        ...
        if self.export_parquet:
            intersections_gdf = gpd.GeoDataFrame(all_intersection_geometries, geometry=self.geom_column, crs=self.gdf.crs)
            intersections_gdf['warning'] = self.message
            intersections_gdf['status'] = 'open'
            intersections_gdf['val_date'] = validation_date
            intersections_gdf.to_parquet(fr"{self.output_dir}\{self.layername}_topology_checks.parquet", engine='pyarrow', compression='zstd', write_covering_bbox=True, row_group_size=50000)

        if self.export_gpkg or self.export_parquet_by_geometry_type:
            if len(intersection_geometries) > 0:
                intersections_gdf = gpd.GeoDataFrame(intersection_geometries, geometry=self.geom_column, crs=self.gdf.crs)
                ...
                intersections_gdf['val_date'] = validation_date
                if self.export_gpkg:
                    intersections_gdf.to_file(fr"{self.output_dir}\topology_checks.gpkg", layer=f"{self.layername}_errors_areas", driver="GPKG")
                if self.export_parquet_by_geometry_type:
                    intersections_gdf.to_parquet(fr"{self.output_dir}\{self.layername}_topology_checks_poly.parquet", engine='pyarrow', compression='zstd', write_covering_bbox=True, row_group_size=50000)
            ...
            if len(intersection_geometries_point) > 0:
                ...
                if self.export_gpkg:
                    intersections_gdf.to_file(fr"{self.output_dir}\topology_checks.gpkg", layer=f"{self.layername}_errors_points", driver="GPKG")
                if self.export_parquet_by_geometry_type:
                    intersections_gdf.to_parquet(fr"{self.output_dir}\{self.layername}_topology_checks_point.parquet", engine='pyarrow', compression='zstd', write_covering_bbox=True, row_group_size=50000)
            ...
            if len(intersection_geometries_line) > 0:
                ...
                intersections_gdf['val_date'] = validation_date
                if self.export_gpkg:
                    intersections_gdf.to_file(fr"{self.output_dir}\topology_checks.gpkg", layer=f"{self.layername}_errors_lines", driver="GPKG")
                if self.export_parquet_by_geometry_type:
                    intersections_gdf.to_parquet(fr"{self.output_dir}\{self.layername}_topology_checks_line.parquet", engine='pyarrow', compression='zstd', write_covering_bbox=True, row_group_size=50000)
            ...
            if len(intersection_geometries_multipolygon) > 0:
                ...
                intersections_gdf['val_date'] = validation_date
                if self.export_gpkg:
                    intersections_gdf.to_file(fr"{self.output_dir}\topology_checks.gpkg", layer=f"{self.layername}_errors_multipolygon", driver="GPKG")
                if self.export_parquet_by_geometry_type:
                    intersections_gdf.to_parquet(fr"{self.output_dir}\{self.layername}_topology_checks_multipolygon.parquet", engine='pyarrow', compression='zstd', write_covering_bbox=True, row_group_size=50000)
    ...
    def run_features_intersect_layer(self, intersect=True, buffer_lines=True):
        ...
        if intersect:
            ...
            gdf = self.find_not_intersections_features_in_layer(buffer_lines=buffer_lines)
        self.save_gdf(gdf)
    ...
    def run_line_not_touches_layer(self, buffer_lines=False):
        ...
        self.read_data_intersect()
        lines_gdf = self.find_not_intersections_touches_on_geom(buffer_lines=buffer_lines)
        self.save_gdf(lines_gdf)
