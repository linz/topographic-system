...
if __name__ == "__main__":
    ...
    process_func = partial(process_table_to_parquet, 
                          data_path=data_path, 
                          export_folder=export_folder, 
                          non_spatial_tables=non_spatial_tables)
    
    # Process tables in parallel
    max_workers = min(round(multiprocessing.cpu_count() / 2), len(tables))  # Don't use more workers than tables
    
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_table = {executor.submit(process_func, table_info): table_info[0] 
        ...
        for future in as_completed(future_to_table):
            ...
            try:
                ...
                print(f"[{completed_count}/{total_tables}] Table {table_name} generated an exception: {exc}")
