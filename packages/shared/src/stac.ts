...
function createBasicStacItem(): StacItem {
  return {
    ...
    links: [],
    properties: {
      // TODO: CHECK THESE PROPERTIES. Consider using STAC Processing extension?
      datetime: CliDate,
...
function addChildDataToParent(
  ...
  const newLinkToChild = <StacLink>{ rel: expectedRel, href: stacChildFile, type: expectedType, ...newLinkStats };
