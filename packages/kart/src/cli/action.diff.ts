...
function getArtifactsUrl(directory: string): string {
  if (!process.env['GITHUB_ACTIONS']) {
    logger.warn('Not running in GitHub Actions, returning local file path');
    return fsa.toUrl(directory).href;
  }
  // await $`gh api repos/${repo}/actions/runs/${runId}/artifacts --jq '.artifacts[] | select(.name=="diff-artifacts") | .url'`;

  const runId = process.env['GITHUB_RUN_ID'];
  const repo = process.env['GITHUB_REPOSITORY'];

  if (runId && repo) {
    logger.info({ directory, runId, repo }, 'Artifacts will be available at GitHub Actions run URL');
    return `https://github.com/${repo}/actions/runs/${runId}#artifacts`;
  } else {
    logger.warn('Missing GITHUB_RUN_ID or GITHUB_REPOSITORY, falling back to local path');
    return fsa.toUrl(directory).href;
  }
}
...
export const diffCommand = command({
  ...
  async handler(args) {
    ...
    // Upload artifacts and get the URL
    const artifactUrl = getArtifactsUrl('diff');
...
function buildMarkdownSummary(
  ...
  summary += '`';
  summary += `, see the HTML file ${htmlFile.pathname} `;
  summary += `in the [attached artifacts](${artifactUrl}).\n\n`;
